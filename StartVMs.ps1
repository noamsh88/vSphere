###############################################################################################
#Script login to vCenter and will start single or multiple VMs according to argumentet entered#
###############################################################################################
$VMList = $args[0]
...
foreach($vm in $VMList.split(','))
{
   ...
   # Starting VM if it powered off
   if ($VMPowerState -like "*PoweredOn*"){
   Write-Host "$vm VM is already Powered ON"
   }
   else
   {
