##############################################################################
#Script login to vCenter and will start all VMs related to host name entered #
##############################################################################
$HostName = $args[0]
...
# Validate all Host VMs are powered on
foreach($vm in $VMList)
{
   ...
   # Exit with error in case VM is on power off state
   if ($VMPowerState -like "*PoweredOff*"){
     Write-Host " $vm VM on $HostName is Powered OFF, exiting.."
     exit 1
   }

}

Write-Host "$HostName VMs started succesfully" -BackgroundColor DarkGreen
